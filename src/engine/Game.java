...
public class Game {
	...
	public void targetCity(Army army, String targetName) {
		army.setMarchFrom(army.getCurrentLocation());
	...
	public void endTurn() {
		...
		for (Army a : player.getControlledArmies()) {
			...
			if (a.getDistancetoTarget() == 0 && !a.getCurrentStatus().equals(Status.IDLE) && !a.getCurrentStatus().equals(Status.BESIEGING)) {
		...
		for(int i=0;i<player.getControlledCities().size();i++) {
			totalUpkeep+=player.getControlledCities().get(i).getDefendingArmy().foodNeeded();
		}
